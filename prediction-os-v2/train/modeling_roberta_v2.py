...
class RobertaForQuestionAnswering_v2(BertPreTrainedModel):
    config_class = RobertaConfig
    base_model_prefix = "roberta"
    ...
    Returns:
        ...
        has_logits = has_logits.squeeze(-1)
        ...
        if start_positions is not None and end_positions is not None:
            ...
            choice_loss = cls_loss_fct(has_logits, is_impossibles)
            
            #total_loss = (start_loss + end_loss) / 2
            total_loss = (start_loss + end_loss) / 2 + choice_loss ## position loss와 answer loss를 1:1로 사용
            outputs = (total_loss,) + outputs
...
class RobertaForQuestionAnswering_v3(BertPreTrainedModel):
    config_class = RobertaConfig
    base_model_prefix = "roberta"
    ...
    Returns:
        ...
        logits = self.qa_outputs(sequence_output)
        start_logits, end_logits = logits.split(1, dim=-1)
        start_logits = start_logits.squeeze(-1).contiguous() # (32,512) # (batch,labels(max_token))
        ...
        outputs = (start_logits, end_logits, has_logits,) + outputs[2:]
        if start_positions is not None and end_positions is not None: # inference 인퍼런스할때 loss 추가하는 측면에서 이게 들어감
            # If we are on multi-GPU, split add a dimension
            if len(start_positions.size()) > 1:
                start_positions = start_positions.squeeze(-1)
            if len(end_positions.size()) > 1:
            ...
            ### new in v3
            # No answer data의 경우, loss=0
            # start/end 위치계산값(logit) 결과를 0번 인덱스에 고정시키면
            # loss_fnc([100, 0, 0, 0, ...], [0]) == 0
            # loss 계산값이 거의 0이 됨.
            dummy_tensor = torch.zeros(start_logits.shape[1]).to(start_logits.device)
            dummy_tensor[0] = 100
            start_logits[is_impossibles==1] = dummy_tensor
            end_logits[is_impossibles==1] = dummy_tensor
            ...
            ## position loss와 answer loss를 1:1로 사용
            total_loss = (start_loss + end_loss) / 2 + choice_loss #total_loss = (start_loss + end_loss) / 2
            outputs = (total_loss,) + outputs
