            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:
                        ...
                        results = evaluate(args, model, tokenizer, prefix=f'{global_step}')
                        for key in sorted(results.keys()):
                            logger.info("  %s = %s", key, str(results[key]))
                        '''
                        # Write the evaluation result on file
                        logger.info("***** Official Eval results *****")
                        with open("{}/eval_result_{}_{}.txt".format(args.output_dir,
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    model_to_save.save_pretrained(output_dir) #pytorch_model.bin , config.json
                    tokenizer.save_pretrained(output_dir) #tokenizer_78b3253a26.model, vocab.txt, special_tokens_map.json, added_tokens.json
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    results = squad_evaluate_v2(examples, predictions)
    return results
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    cached_features_file = os.path.join(
        ...
        "cached_{}_{}_{}_{}_{}".format( 
            ...
            list(filter(None, args.model_name_or_path.split("/"))).pop(),
            '_'.join(args.train_file.split('.')[0].split('_')[2:]),
            str(args.max_seq_length),str(args.noans_prop)
        ),
    )
...
def main():
    ...
    parser.add_argument("--noans_prop", type=int, default=None)
    parser.add_argument("--freeze_base_model", action='store_true')
    parser.add_argument("--freeze_step", type=int, default=0, help="available 0 ~ 11")
    ...
    logger.info("-------------------------------------------START-------------------------------------------")
    logger.info(f"Train_file:{args.train_file}, EPOCH:{args.num_train_epochs}, output_dir: {args.output_dir}\n")
    # Set seed
    set_seed(args)
